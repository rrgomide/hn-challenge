...
describe('AIService', () => {
  ...
  describe('createAIService', () => {
    it('should prioritize Google AI when GOOGLE_AI_API_KEY is available', async () => {
      ...
      const { createAIService } = await import('../aiService.js')
    ...
    it('should throw error when no API keys are available', async () => {
      const { createAIService } = await import('../aiService.js')
      expect(() => createAIService()).toThrow('No AI provider API keys found. Please set either GOOGLE_AI_API_KEY or OPENAI_API_KEY environment variable.')
    })
  ...
  describe('GoogleAIService', () => {
    ...
    it('should summarize text correctly', async () => {
      ...
      const { GoogleAIService } = await import('../aiService.js')
      const service = new GoogleAIService('test-key')

      const result = await service.summarizeText('This is a long text that needs to be summarized')

      expect(result).toBe('This is a test summary')
      expect(mockGenerateText).toHaveBeenCalledWith({
        model: mockGoogleModel,
        prompt: expect.stringContaining('Summarize the following text in a concise way')
      })
    })
    ...
    it('should handle AI service errors gracefully', async () => {
      ...
      await expect(service.summarizeText('Test text'))
  ...
  describe('OpenAIService', () => {
    ...
    it('should summarize text correctly', async () => {
      ...
      const { OpenAIService } = await import('../aiService.js')
    ...
    it('should handle AI service errors gracefully', async () => {
      ...
      await expect(service.summarizeText('Test text'))
        .rejects.toThrow('Failed to generate summary: OpenAI service error')
    })
